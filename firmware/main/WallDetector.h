...
public:
  ...
  bool restore() {
    ...
    log_i("WallDetector Restore: %d %d %d %d", wall_ref.side[0],
...
private:
  ...
  void calibration_front() {
    ...
    printf("Wall Calibration Front:\t%04d\t%04d\n", (int)wall_ref.front[0],
           (int)wall_ref.front[1]);
  }
  void task() {
    ...
    while (1) {
      ...
      for (int i = 0; i < 2; i++) {
        for (int j = ave_num - 1; j > 0; j--)
          side_buf[j][i] = side_buf[j - 1][i];
        side_buf[0][i] = ref.side(i);
        int sum = 0;
        for (int j = 0; j < ave_num; j++)
          sum += side_buf[j][i];
        sum /= ave_num;
        if (sum > WALL_DETECTOR_THRESHOLD_SIDE * 1.1f)
          wall[i] = true;
        else if (sum < WALL_DETECTOR_THRESHOLD_SIDE * 0.9f)
          wall[i] = false;
      }

      for (int i = 0; i < 2; i++) {
        distance.side[i] = ref.side(i) - wall_ref.side[i];
        distance.front[i] = ref.front(i) - wall_ref.front[i];
      }
