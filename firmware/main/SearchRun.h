...
private:
  ...
  void wall_avoid(const float distance) {
    ...
    for (int i = 0; i < 2; i++) {
      if (prev_wall[i] && !wd.wall[i] && sc.position.x > 30.0f) {
        ...
        if (distance > 90 - 1)
          sc.position.x =
              sc.position.x - ((int)sc.position.x) % 90 + 76 - ahead_length;
      ...
      if (!prev_wall[i] && wd.wall[i] && sc.position.x > 30.0f) {
        ...
        if (distance > 90 - 1)
          sc.position.x =
              sc.position.x - ((int)sc.position.x) % 90 + 64 - ahead_length;
  ...
  void wall_calib(const float velocity) {
    ...
    if (wd.wall[2]) {
      float value =
          tof.getDistance() - (10 + tof.passedTimeMs()) / 1000.0f * velocity;
