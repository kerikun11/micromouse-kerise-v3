...
public:
  ...
  float getNorm() const { return sqrt(x * x + y * y); }
  ...
  void task() {
    ...
    while (1) {
      ...
      differential.trans = (target.trans - target_prev.trans) /
                               SPEED_CONTROLLER_PERIOD_US * 1000000 -
                           accel[0];
      //        differential.rot = (target.rot - target_prev.rot) /
      //        SPEED_CONTROLLER_PERIOD_US * 1000000 - imu.angular_accel;
      differential.rot =
          (target.rot - target_prev.rot) / SPEED_CONTROLLER_PERIOD_US *
              1000000 -
          (gyro[0] - gyro[1]) / SPEED_CONTROLLER_PERIOD_US * 1000000;
