...
class LED: TaskBase {
  public:
    LED(int pin0, int pin1, int pin2, int pin3): TaskBase("LED", LED_TASK_PRIORITY, LED_STACK_SIZE), pin0(pin0), pin1(pin1), pin2(pin2), pin3(pin3) {
      pinMode(pin0, OUTPUT);
      pinMode(pin1, OUTPUT);
      pinMode(pin2, OUTPUT);
      pinMode(pin3, OUTPUT);
    ...
    uint8_t operator=(uint8_t value) {
      digitalWrite(pin0, (value & 0x01) ? HIGH : LOW);
      digitalWrite(pin1, (value & 0x02) ? HIGH : LOW);
      digitalWrite(pin2, (value & 0x04) ? HIGH : LOW);
      digitalWrite(pin3, (value & 0x04) ? HIGH : LOW);
      return value;
    }
  private:
